...
docker build --build-arg GRAFANA_VERSION=${_grafana_version} --tag "xarorsa/grafana:${_grafana_tag}"  --no-cache=true .

if [ $_release_build == true ]; then
	docker build --build-arg GRAFANA_VERSION=${_grafana_version} --tag "xarorsa/grafana:latest"  --no-cache=true .
fi
